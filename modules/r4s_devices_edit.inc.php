...
*/
  ...
  //updating 'DEVICE_NAME' (varchar)
   $rec['DEVICE_NAME']=gr('device_name');
  //updating 'DEVICE_STATUS' (varchar)
   $rec['DEVICE_STATUS']=gr('device_status');
  ...
  if ($this->tab=='data') {
   ...
   if ($this->mode=='update') {
	...
	if ($title_new) {
	 ...
	 $new_id=SQLInsert('r4s_values',$prop);
	}
   }
   global $delete_id;
   if ($delete_id) {
    SQLExec("DELETE FROM r4s_values WHERE ID='".(int)$delete_id."'");
   }
   $properties=SQLSelect("SELECT * FROM r4s_values WHERE DEVICE_ID='".$rec['ID']."' ORDER BY ID");
   ...
   for($i=0;$i<$total;$i++) {
    ...
    if ($this->mode=='update') {
      ...
      SQLUpdate('r4s_values', $properties[$i]);
